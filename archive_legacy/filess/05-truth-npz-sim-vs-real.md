...
Write these into the code as explicit preconditions and **assert them at generation time**. If any one changes, the answer changes with it.
...
| 4 | Nothing in `backend/` may reference `site.a`, `site.c`, `S_MAX` or `C_KNOTHE` — **test T18** greps for these |
...
**EC-8 — The simulator control panel reads `truth/`.** Under the old scheme it showed the nearest 30-minute frame, so the truth surface visibly **stepped** while node data moved smoothly. Under `truth_at(t)` it is continuous at any speed multiplier. **Better demo, less code.**
...
| **`a`, `c` parameter values** | Chosen by us: 0.65, 0.01414 | Site-specific, estimated from history or **learned from sensors** | Our PINN learns them. That is the actual product feature, not a simulator artefact |
...
| **Radio delivery** | Log-distance path loss + shadowing + PDR draw | Real RF | `n` and `L_clutter` are **calibrated, not derived**. Validate against Meshtasticator and report the disagreement |
...
| **Node deaths** | Scripted `node_kill` / `cluster_kill` | Vandalism, cattle, flooding, ground failure | Mechanism is identical: a node stops arriving. The *cause* is what we correctly refuse to infer |
| **Truth for scoring** | `truth.npz` | **Does not exist** | Replaced by: `loo_residual` (live), periodic levelling survey (weeks), InSAR Sentinel-1 (12-day repeat, cm-scale) |
| **Ground-truth validation** | RMSE against `truth_at(t)` | Levelling survey + InSAR | Our claim is *"here is our error against a known answer, and here is a proxy that tracks it without one."* Both numbers, side by side |

### 7.1 What we will say on stage

> *"Everything the backend sees is what a real deployment would produce: 21-byte packets, arrival provenance, gaps where the radio failed, and the mine's own blast register. The only thing that exists here and not at a real mine is the answer key — and we deliberately gave the backend no way to reach it, enforced by a test in CI rather than by a promise. The accuracy number on the dashboard is the one that still works when the answer key goes away."*
...
| **T15** | Both anchors read `|S| < 1 mm` in truth at every check time |
| **T18** | No file under `backend/` references `site.a`, `site.c`, `S_MAX`, `C_KNOTHE`, or `truth` |
| **T25** | `meta.generator_sha256` in `truth.npz` matches the actual hash of the generator source at load |
| **T26** | The four separability preconditions in §2.3 are asserted at generation time and raise if violated |

---

## 9. Six lines to say out loud

> The ground truth is not a recording, it is an analytic field — so we store the field, not nineteen hundred photographs of it, and we can score at any instant rather than only at the frames we happened to save. That is 65 kilobytes instead of 126 megabytes, and it makes our error number *cleaner*, because there is no interpolation between frames to contaminate it. The backend has no import path to that file, enforced by a test in continuous integration rather than by a promise, because the failure it prevents — a system that quietly replays the answer key while still looking correct — is invisible from the outside. And we are explicit that at a real mine this file does not exist at all: what survives is the leave-one-out residual, the levelling survey, free Sentinel-1 InSAR, and two anchor nodes that must read zero forever. Those four are what a real deployment cross-checks against, and our accuracy claim is built to survive on them alone.
