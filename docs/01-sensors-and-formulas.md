...
> **This file owns:** what physically sits on a node, what each sensor outputs, the one formula that generates the ground, every derivation from it, the generators for channels *not* derived from it, the six-stage corruption chain, and the argument that lets us generate millions of correct rows.
>
> **This file does NOT own:** radio behaviour (→ 02), file schemas (→ 03, 04), the PINN (→ 03), the alarm (→ 06).
>
> **v2.0 changes:** peak magnitudes recomputed and corrected (§3.6 of file 00); `t_crack` moves to 8.93 days; the packet is now 23 bytes; new tests T4, T5, T6.
...
|---|---|---|---|---|---|---|---|
| 1 | Tilt / inclination | MPU9250 accelerometer | direction of gravity vs the board → ground slope | `tilt_x`, `tilt_y` | int16, **LSB 2 µrad** | ±65 534 µrad (±3.75°) | **Secondary vote.** Thermal drift can rival the signal in the far field. Never alarms alone |
...
| 3 | Strain gauge | foil gauge + ADS1115 | surface stretch/squash across panel width | `strain_ue` | int16, 1 µε | ±32 767 µε | **Primary signal.** This is what triggers |
| 4 | Wire extensometer | invar wire, 10 m, to anchor peg | 3-D distance change between two pegs | `ext_delta_10um` | int16, ×10 µm | ±327.6 mm | **Primary signal.** Directly answers the PS bullet "change in relative distance between nodes" |
...
| — | Radio link | LoRa PHY, **measured at the gateway** | quality of the hop that just arrived | `rssi_dbm`, `snr_db`, `hops` | int16, f32, uint8 | −140 → −40 dBm | Provenance. Feeds **σ**, never feeds the PINN |

**Rows 1–7 are on the node. The radio row is measured at the gateway** — a node cannot measure its own journey.
...
|---|---|---|
| ESP32 + SX1262 | brain + voice | sets the **23-byte** packet ceiling, which sets everything about the cadence |
...
| Gateway (RPi + 4G + **10 m mast** + siren + SD) | collect, alert, buffer | siren and SMS are **actions**, not measurements — they appear in no file |
...
| Pack 23 bytes + CRC | every 60 s | < 50 µs | 23 B |
| LoRa TX @ SF7/BW125 | every 60 s | **90.4 ms** radio-on | — |
| **Un-ACKed frame ring, 72 h** | persistent | — | **99 KB flash** |

**CPU duty ≈ 0.3%. Peak RAM ≈ 4 KB of 520 KB. Flash ring 99 KB of megabytes.**
...
The one exception is the tiny on-board trip detector that fires a P3 event packet (file 02 §4.1). It is two comparisons and a subtraction, it raises no alarm, and it exists only to change *when* the gateway looks.
...
**Where the run actually sits:** `g(40 d) = 0.432`. After the full 40-day scenario the trough is only **43% settled**, peak depth 0.842 m. That is realistic and it is why the crack demo and the alarm demo both land in the middle of the run rather than at the end.
...
### 2.4 Where the extrema sit — derived, and the source of the sizing rule

Because the profile is an erf pair, the extrema have closed forms:

```
max |P′|      at the panel edge            = 1/r
max |Q″|      at  y = y_edge ± r/√(2π)     (= ±29.9 m for r = 75)
curvature falls to 10% of peak at |x| = 1.1r  →  the steep band is ~2r wide
```

That last line is what sets node spacing in file 02 §2. **The sampling rule is a consequence of this formula, not a survey convention.**

### 2.5 The corrected peak magnitudes

**v1.4 had these wrong.** Recomputed on a 4001 × 4001 grid over `x ∈ [0, 900]`, `y ∈ [−50, 450]`:

| Channel | Fully settled | At day 40 | Wire capacity | Margin |
|---|---|---|---|---|
| `S` | 1.950 m | 0.842 m | not on wire | — |
| `tilt_x` | 25 978 µrad | 11 222 µrad | ±65 534 µrad | 2.52× |
| `tilt_y` | 26 000 µrad | 11 232 µrad | ±65 534 µrad | 2.52× |
| `strain` | **12 649 µε** ~~8 300~~ | 5 464 µε | ±32 767 µε | **2.59×** |
| `ext` (10 m) | **126.5 mm** ~~83~~ | 54.6 mm | ±327.6 mm | **2.59×** |

Everything still fits the wire with comfortable margin. The design survives; the numbers on the slide change.

### 2.6 A free test that catches sign errors instantly
...
with θ_c = 1500 µε and B·|Q″|·S_max = 12 649 µε:
t_crack = −(1/0.01414) · ln(1 − 0.1186) = 8.93 days
```

> **Test T10:** the crack latch found by simulation must match this analytic time to within one sample interval. It catches a wrong sign or a wrong `B` in one second. **With the old 8 300 µε figure this test would have expected day 14.1 and passed against a wrong generator.**
...
| `epoch_lo` | the **beacon** | network time, not measurement |
| `rssi`, `snr`, `hops` | mesh Layer 4 (→ 02) | the radio journey, not the ground |
| `alive`, `seq` gaps | mesh Layer 4 (→ 02) | delivery, not measurement |
...
| 3 | White noise | `v += σ_w · sigma_scale_i · ξ` | ❌ no — this *is* σ's floor |
...
> **Inversion runs in reverse: undo stage 4 (sag), then stage 1 (thermal).** File 06 §2.2 has the code. Getting this backwards leaves a systematic residual of `k_T·(T−T_ref)·(V_nom/V − 1)` that looks exactly like a physics bug.
...
> **Test T6 — new in v2.0.** Hash the ordered list of stage names actually applied and assert it against the constant. A chain that silently reorders after a refactor is invisible to every other test.

### 4.2 ELI5

Think of the clean value as a clean shirt. Five things happen to it, in this order: it gets a heat stain (removable), it gets slowly wrinkled (not removable), someone flicks dust on it (not removable), it shrinks in the wash (removable), and then it gets folded into a fixed-size box that rounds off the shape (not removable).

C7 later un-shrinks it and removes the heat stain. The wrinkles and dust it cannot remove, so instead it **measures how bad they are and reports that number as σ**. That honesty is the whole design.
...
> **Test T5 — new in v2.0.** Assert `S(t)/S(t′) = g(t)/g(t′)` at every grid cell for a few random `t` pairs. If a future edit makes the surface non-separable, T5 fails immediately instead of silently invalidating `truth.npz`.
...
| Rows produced | 1.79 M | 12.96 M |
| **`nodes.csv` on disk** | **8.0 MB** | **26 MB** | ← *bounded by retention, not by run length* |
| Archive parquet | ~45 MB | ~330 MB |
...
3. **~~CSV at 150 MB~~ — solved.** The retention model (file 04 §4.8) makes `nodes.csv` a function of node count and window length, not run length. It is 8 MB on day 3 and 8 MB on day 300.
...
flowchart TD
  ...
  D3 --> D5["latch on θ_c → crack bits<br/>fires day 8.93"]
  ...
  L3 --> PK["pack <b>23 bytes</b><br/>incl. epoch_lo"]
  PK --> L4["<b>LAYER 4 — mesh radio</b><br/>see file 02"]
  L4 --> CSV["nodes.csv, 36 h window<br/>+ parquet archive<br/>see file 04"]
...
| Test | Asserts |
|---|---|
| **T1** | Volume conservation: `∫∫ S_final dx dy ≈ a · m_seam · panel_area`, within 1%. *Measured: ratio 1.0000* |
| **T2** | `S_final → 0` beyond `r` outside the panel. *Measured: anchors read 8.7e−5 mm and 7.9e−6 mm* |
| **T3** | Closed-form derivatives match central finite differences of `S` to < 1e-7 |
| **T4** | Peak magnitudes reproduce 00 §3.6 to ±2%: tilt 25 978 µrad, strain 12 649 µε, ext 126.5 mm |
| **T5** | Separability: `S(t)/S(t′) = g(t)/g(t′)` at every grid cell |
| **T6** | The corruption chain's applied stage order hashes to the frozen constant |
| **T7** | Undoing corruption stages 4 then 1 with true `T_chip`, `V_bat` leaves residual inside the white-noise band |
| **T10** | Simulated crack latch matches `t_crack = 8.93 days` within one sample |

---

## 8. Six lines to say out loud

> Each node carries seven cheap sensors and an ESP32 running at 0.3% CPU duty — compute is free, airtime is not, which is why the packet is twenty-three bytes. Two of those seven sensors do not measure the ground at all; they exist so the other four can be un-corrupted later. All four ground channels are derivatives of one closed-form Knothe surface, so there is no second generator and no way for them to be quietly inconsistent — we test that by checking volume conservation and by predicting analytically when the first crack latches, which is day nine. Vibration, temperature and battery come from independent physical models, because the ground cannot explain a passing truck. Six corruption stages run in a fixed order specifically so the backend can invert two of them and turn the rest into an honest uncertainty. And because the surface is separable in space and time, the truth is one bowl and one exponential rather than nineteen hundred photographs — which is why millions of rows fit on a laptop and why a rerun is byte-identical.
